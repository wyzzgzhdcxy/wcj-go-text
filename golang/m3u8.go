...
func DownloadM3u8(url string, fn string, dir string, threadCount int) {
	...
	cmd := core.Command("N_m3u8DL-RE", url,
...
func DownloadM3u8WithProgress(url string, fn string, dir string, threadCount int, progressCallback func(status string, percent float64, speed string)) {
	...
	// stdout 和 stderr 必须并发排空：若串行读，子进程在 stdout 空闲时写满
	// stderr 管道会被阻塞，造成双管道死锁。
	lines := make(chan string, 128)
	var readers sync.WaitGroup
	readers.Add(2)
	for _, pipe := range []io.ReadCloser{stdout, stderr} {
		go func(r io.ReadCloser) {
			defer readers.Done()
			scanner := bufio.NewScanner(transform.NewReader(r, simplifiedchinese.GBK.NewDecoder()))
