// Package sqllite 集中管理项目内全部 SQLite 数据（统一到 sync_list.db）。
...
//     music_*/git_*/sync_logs）统一存在 sync_list.db 内，按业务域分文件管理 schema 与 API，
//     调用方不应直接使用 *sql.DB 句柄。
package sqllite
...
func runLegacyMigration(target *sql.DB) {
	...
	for _, p := range plan {
		...
		if err := tx.Commit(); err != nil {
			...
			fmt.Printf("[sqllite] 已迁移 %s -> sync_list.%s，%d 行\n", p.file, p.table, inserted)
		}
		stmt.Close()
		rows.Close()
		src.Close()
