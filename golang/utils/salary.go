...
func SalaryList(request SalaryReq) SalarySummaryVo {
	...
	for i := 1; i <= 12; i++ {
		...
		// 设置免税额
		exemption := new(big.Float).Set(exemptionAmount)
		exemption.SetPrec(2)
		salaryVo.ExemptionAmount = exemption
		...
		if taxRealSalary.Cmp(salaryVo.ExemptionAmount) <= 0 {
			...
			tax := new(big.Float).Set(taxAmount)
			tax.SetPrec(2)
			salaryVo.TaxAmount = tax
		} else {
			...
			taxValue.SetPrec(2)
			salaryVo.Tax = taxValue

			readyTax.Add(readyTax, salaryVo.Tax)
		}

		readyTaxValue := new(big.Float).Set(readyTax)
		readyTaxValue.SetPrec(2)
		salaryVo.ReadyTax = readyTaxValue

		realSalary := new(big.Float).Sub(taxRealSalary, salaryVo.Tax)
		realSalary.SetPrec(2)
		salaryVo.RealSalary = realSalary
...
func computerSocialBx(socialSecurityBase *big.Float, salaryVo *SalaryVo) {
	...
	// 设置精度（修复点4：修正精度设置方式）
	setPrec := func(f *big.Float, prec uint) *big.Float {
		return new(big.Float).SetPrec(prec).Set(f)
	}

	// 更新到结构体（修复点5：使用正确精度的值）
	salaryVo.Yanglaobx = setPrec(yanglao, 2)
	salaryVo.Yilbx = setPrec(yiliao, 2)
	salaryVo.Shiyebx = setPrec(shiye, 2)
	salaryVo.Shengyubx = new(big.Float).SetFloat64(0) // 生育保险固定0
	salaryVo.Gjj = setPrec(gongjijin, 0)              // 公积金取整
	...
	salaryVo.SocialBx = setPrec(socialBx, 2)
...
func summaryInfo(salaryInfoVo *SalarySummaryVo) {
	...
	// 设置精度（保留2位小数，四舍五入）
	totalSalary.SetPrec(2)
	socialBxTotal.SetPrec(2)
	totalRealSalary.SetPrec(2)

	// 更新到汇总对象
	salaryInfoVo.TotalSalary = totalSalary
	salaryInfoVo.SocialBxTotal = socialBxTotal
	salaryInfoVo.TotalRealSalary = totalRealSalary
}
