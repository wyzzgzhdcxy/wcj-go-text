...
var (
	...
	isDownloading  bool
	// cancelRequested 标记用户是否请求取消当前下载。
	// Windows 上 Process.Kill 后 ExitCode 是 1 而不是 -1，不能用退出码判断取消。
	cancelRequested atomic.Bool
)

// setCurrentProcess 记录当前下载进程（Start 成功后调用，保证 Process 非 nil）
func setCurrentProcess(cmd *exec.Cmd) {
	ytdlpMutex.Lock()
	currentProcess = cmd
	ytdlpMutex.Unlock()
}

// clearCurrentProcess 清除当前下载进程记录
func clearCurrentProcess() {
	ytdlpMutex.Lock()
	currentProcess = nil
	ytdlpMutex.Unlock()
}

// killCurrentProcess 终止当前下载进程（若存在）
...
// GetAllTasksJSON 返回所有任务的 JSON 快照。
// 在锁内对任务做值拷贝再序列化，避免后台协程更新进度时与序列化并发产生数据竞争。
func (m *DownloadTaskManager) GetAllTasksJSON() (string, error) {
	m.mutex.RLock()
	defer m.mutex.RUnlock()
	tasks := make([]DownloadTask, 0, len(m.tasks))
	for _, task := range m.tasks {
...
func startDownloadTask(taskID string) {
	...
	// 实际执行下载
	err := executeDownload(task.URL, task.FormatID, task.OutputPath, task.Platform, func(progress ProgressInfo, logMsg string) {
		if logMsg != "" {
	...
	if err != nil {
		log.Printf("[Task %s] 下载失败: %v", taskID, err)
	}
	// 写入最终状态，避免任务永久停留在 downloading
	GetTaskManager().UpdateTask(taskID, func(t *DownloadTask) {
...
func executeDownload(url string, formatID string, outputPath string, platform string, progressCallback func(ProgressInfo, string)) error {
	...
	cmd := core.Command(ytdlpPath, args...)
	...
	// 先等 stderr 读完（进程退出后写端关闭，Read 会自然返回 EOF）再 Wait，
	// 避免过早关闭管道丢失末尾输出或在 Windows 上泄漏阻塞中的读协程
	<-scanDone
	err = cmd.Wait()
	clearCurrentProcess()
...
func DownloadVideo(url string, options YtDlpOptions, progressCallback func(ProgressInfo)) error {
	...
	// 先等 stderr 读完再 Wait，避免丢失末尾输出或在 Windows 上泄漏阻塞中的读协程
	<-scanDone
	err = cmd.Wait()
	clearCurrentProcess()
...
func CheckDependencies() map[string]bool {
	...
	// Check yt-dlp
	ytdlpPath := FindYtDlp()
	cmd := core.Command(ytdlpPath, "--version")
	err := cmd.Run()
	result["yt-dlp"] = err == nil

	// Check ffmpeg
	ffmpegCmd := core.Command("ffmpeg", "-version")
	err = ffmpegCmd.Run()
