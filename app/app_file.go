...
func splitFileBySize(filePath, outputDir string, chunkSize int64) ([]string, error) {
	if chunkSize <= 0 {
		return nil, fmt.Errorf("分块大小必须大于0")
	}
	...
	// 用固定缓冲流式复制，避免按分块大小一次性申请内存（如 10GB 分块会占满内存）
	buf := make([]byte, min64(1024*1024, chunkSize))
	var files []string
	idx := 0
	for {
		idx++
		partPath := filepath.Join(outputDir, fmt.Sprintf("%s_part%d%s", base, idx, ext))
		part, werr := os.OpenFile(partPath, os.O_WRONLY|os.O_CREATE|os.O_TRUNC, 0o644)
		if werr != nil {
			return nil, werr
		}
		n, cerr := io.CopyBuffer(part, io.LimitReader(src, chunkSize), buf)
		closeErr := part.Close()
		if cerr != nil {
			return nil, cerr
		}
